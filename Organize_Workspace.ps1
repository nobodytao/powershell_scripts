...
$cmdAmount = $cmdProcesses.Length

#Set width and height for cmd-windows
$cmdWidth = [Math]::Floor(9*$screenWidth/20)
$cmdHeight = [Math]::Floor($screenHeight/$cmdAmount)
$Ycoord = 0
$Xcoord = $screenWidth - $cmdWidth


# Arrange all opened cmd-windows at the right side of screen
foreach ($cmdProcess in $cmdProcesses)
    {
    $cmdId = $cmdProcess.Id
    
    Set-Window -Id $cmdId -X $Xcoord -Y $Ycoord -Width $cmdWidth -Height $cmdHeight -Passthru
    
    $Ycoord = $Ycoord + $cmdHeight
    }


# Find all edge.exe processes
$msedgeProcesses = Get-Process -Name 'msedge'

#Set width and height for msedge-windows
$msedgeWidth = $screenWidth - $cmdWidth
$msedgeHeight = $screenHeight
$msedgeYcoord = 0
$msedgeXcoord = 0

# Arrange all opened msedge-windows at the left side of screen
foreach ($msedgeProcess in $msedgeProcesses)
    {
        if ($msedgeProcess.MainWindowHandle -ne 0)
        {
        $msedgeId = $msedgeProcess.Id
        Set-Window -Id $msedgeId -X $msedgeXcoord -Y $msedgeYcoord -Width $msedgeWidth -Height $msedgeHeight -Passthru
        }
    }